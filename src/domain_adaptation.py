...
class DomainAdaptationConfig:
    ...
    # Version 1 supports target_only. Joint adaptation is added later.
    ...
    def validate(self) -> None:
        ...
        if self.adaptation_mode != "target_only":
            raise ValueError(
                "Version 1 implements adaptation_mode='target_only' only. "
                "Add joint source/target training after the pilot is verified."
            )
...
def train_one_batch(
    ...
    if images.ndim != 4 or images.shape[1] != 3:
        raise RuntimeError(f"Expected images [B,3,H,W], got {tuple(images.shape)}")
    if masks.ndim != 3:
        raise RuntimeError(f"Expected masks [B,H,W], got {tuple(masks.shape)}")

    unique_values = torch.unique(masks)
    if not set(unique_values.detach().cpu().tolist()).issubset({0, 1}):
        raise RuntimeError(f"Masks contain unexpected labels: {unique_values.tolist()}")
...
def run_smoke_test(
    ...
    if source_train is not None:
        ...
        source_images, source_masks = next(iter(source_loader))
        logger.info(
        ...
        logger.info("Smoke source optimisation loss: %.6f", source_loss)
    ...
    logger.info("Smoke target optimisation loss: %.6f", target_loss)
